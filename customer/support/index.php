...
while ($b = mysqli_fetch_assoc($bus_res)) $businesses[] = $b;
...
while ($d = mysqli_fetch_assoc($del_res)) $delivery[] = $d;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_ticket'])) {
    ...
    if (empty($subject) || empty($message)) {
    ...
    } elseif ($send_to == 'business' && $business_id == 0) {
        $error = "Please select a business.";
    } elseif ($send_to == 'delivery' && $agent_id == 0) {
        ...
        $ticket_no = 'CUS-' . date('Ymd') . '-' . strtoupper(substr(uniqid(), -6));
        $assigned_to_type = 'admin';
        $assigned_to_id = 'NULL';

        if ($send_to == 'business' && $business_id > 0) {
            $assigned_to_type = 'business';
            $assigned_to_id = $business_id;
        } elseif ($send_to == 'delivery' && $agent_id > 0) {
        ...
        $sql = "INSERT INTO support_tickets 
                ...
                VALUES ('" . esc($ticket_no) . "', 'customer', $customer_id, '$assigned_to_type', $assigned_to_id,
...
$rec_sql = "SELECT t.*, 
            ...
            WHERE (t.created_by_type = 'customer' AND t.created_by_id = $customer_id)
               OR (t.assigned_to_type = 'customer' AND t.assigned_to_id = $customer_id)
...
<head>
    ...
    <title>Customer Support | UNK System</title>
    ...
    <style>
        * {
        ...
        .last-sender.business {
            color: #e67e22;
        }
        
        .last-sender.delivery {
            color: #3498db;
        }
...
<div class="customer-content">
    <div class="page-header">
        <h1><i class="fas fa-headset"></i> Customer Support Center</h1>
        <p>Get help from Admin, contact businesses or delivery agents</p>
    </div>
    ...
    <div class="support-layout">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="faq-item"><div class="faq-question" onclick="toggleFAQ(this)"><span>📦 How do I place an order?</span><i class="fas fa-chevron-down"></i></div><div class="faq-answer">Browse products, add to cart, proceed to checkout, enter address, choose payment, confirm order.</div></div>
                <div class="faq-item"><div class="faq-question" onclick="toggleFAQ(this)"><span>📍 How can I track my order?</span><i class="fas fa-chevron-down"></i></div><div class="faq-answer">Go to "My Orders", click the order, then "Track Order" for live updates.</div></div>
                <div class="faq-item"><div class="faq-question" onclick="toggleFAQ(this)"><span>💰 What payment methods are accepted?</span><i class="fas fa-chevron-down"></i></div><div class="faq-answer">Cash on Delivery, M‑Pesa, Tigo Pesa, Airtel Money, Bank Transfer, Card.</div></div>
                <div class="faq-item"><div class="faq-question" onclick="toggleFAQ(this)"><span>🚚 How long does delivery take?</span><i class="fas fa-chevron-down"></i></div><div class="faq-answer">1‑3 business days within Dar es Salaam. Same‑day for orders before 12pm.</div></div>
                <div class="faq-item"><div class="faq-question" onclick="toggleFAQ(this)"><span>❌ How do I cancel my order?</span><i class="fas fa-chevron-down"></i></div><div class="faq-answer">Cancel from "My Orders" if still pending or confirmed. Contact support for help.</div></div>
                <div class="faq-item"><div class="faq-question" onclick="toggleFAQ(this)"><span>🔄 What is your return policy?</span><i class="fas fa-chevron-down"></i></div><div class="faq-answer">7‑day return for defective products. Contact support within 7 days of delivery.</div></div>
            </div>
        </div>

        <!-- Ticket Submission Form -->
        <div class="card">
            <div class="card-header"><h3><i class="fas fa-paper-plane"></i> Submit a Ticket</h3></div>
            <div class="card-body">
                <form method="POST" enctype="multipart/form-data">
                    <div class="form-group"><label>Your Name</label><input type="text" class="form-control" value="<?php echo htmlspecialchars($customer['first_name'] . ' ' . $customer['last_name']); ?>" readonly style="background:#f8fafc;"></div>
                    <div class="form-group"><label>Email</label><input type="email" class="form-control" value="<?php echo htmlspecialchars($customer['email']); ?>" readonly style="background:#f8fafc;"></div>
                ...
                <table class="ticket-table">
                    ...
                    <tbody>
                        <?php foreach ($recent_tickets as $t):
                            $last_sender = '';
                            if ($t['last_sender_type'] == 'customer') $last_sender = 'You';
                            elseif ($t['last_sender_type'] == 'admin') $last_sender = 'Admin';
                            elseif ($t['last_sender_type'] == 'business') $last_sender = 'Business';
                            elseif ($t['last_sender_type'] == 'delivery') $last_sender = 'Delivery';
                            else $last_sender = 'System';
                            $sender_class = $t['last_sender_type'] ?? 'customer';
                            $recipient = $t['recipient_name'] ?? 'Unknown';
                            if ($t['assigned_to_type'] == 'admin') $recipient = 'Admin';
                        ?>
                        <tr onclick="window.location.href='ticket.php?id=<?php echo $t['id']; ?>'" style="cursor: pointer;">
                            ...
                            <td><a href="ticket.php?id=<?php echo $t['id']; ?>" class="btn-outline" style="padding: 0.2rem 0.6rem; font-size: 0.65rem;" onclick="event.stopPropagation();">View →</a></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
    ...
    <?php else: ?>
        <div class="tickets-section">
            ...
            <div class="empty-state">
                ...
                <p>No tickets yet. Submit your first support ticket above.</p>
            </div>
        </div>
    <?php endif; ?>
    ...
    <div class="resources-section">
        ...
        <div class="resources-grid">
            <a href="../orders/track.php" class="resource-link"><i class="fas fa-map-marker-alt"></i> Track Your Order</a>
            <a href="../orders/index.php" class="resource-link"><i class="fas fa-list"></i> My Orders</a>
            <a href="../settings/profile.php" class="resource-link"><i class="fas fa-user-edit"></i> Update Profile</a>
            <a href="../settings/address.php" class="resource-link"><i class="fas fa-map-pin"></i> Manage Addresses</a>
            <a href="../products/index.php" class="resource-link"><i class="fas fa-store"></i> Continue Shopping</a>
            <a href="../settings/change-password.php" class="resource-link"><i class="fas fa-key"></i> Change Password</a>
...
function toggleGroups() {
    ...
    businessGroup.style.display = (val === 'business') ? 'block' : 'none';
    deliveryGroup.style.display = (val === 'delivery') ? 'block' : 'none';
    if (businessGroup.querySelector('select')) businessGroup.querySelector('select').required = (val === 'business');
