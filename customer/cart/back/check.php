        ...
        try {
            ...
            <head>
                <style>
                    ...
                    .order-info { background: #f8fafc; padding: 15px; border-radius: 12px; margin: 15px 0; font-size: 14px; }
            ...
            <body>
                <div class="container">
                    ...
                    <div class="order-info">
                        ...
                        <p>Total Amount: TSh ' . number_format($total, 0, '.', ',') . '</p>
                    </div>
...
define('DISTANCE_MATRIX_API_KEY', 'p4YMCETDj0oWcyAyzPui2Z04TSRyqXfvWY9MyUT2XrwAn1cMLoFzuVqPuyI9YEWt');
define('DISTANCE_MATRIX_URL', 'https://api.distancematrix.ai/maps/api/distancematrix/json');
...
// ============================================
// HANDLE AJAX REQUEST - Place Order & Send OTP
// ============================================
...
if ($is_ajax && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($error)) {
        ...
        if ($order_success && $first_order_id > 0) {
            ...
            // Delete old OTPs for order verification
            mysqli_query($conn, "DELETE FROM otp_verifications WHERE email = '$user_email' AND purpose = 'order_verification'");
            
            // Save OTP
            $expires_at = date('Y-m-d H:i:s', strtotime('+10 minutes'));
            $insert_sql = "INSERT INTO otp_verifications (email, otp_code, purpose, reference_id, expires_at, created_at) 
                           VALUES (?, ?, 'order_verification', ?, ?, NOW())";
            $stmt = mysqli_prepare($conn, $insert_sql);
            mysqli_stmt_bind_param($stmt, 'ssis', $user_email, $otp, $first_order_id, $expires_at);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_close($stmt);
            
            // Store order data in session for verification
            $_SESSION['temp_order_id'] = $first_order_id;
            $_SESSION['temp_order_email'] = $user_email;
            $_SESSION['temp_order_total'] = $grand_total;
            $_SESSION['temp_order_ids'] = $order_ids;
            
            // Send OTP email
            if (sendOTPEmail($user_email, $otp, $customer_name, $first_order_id, $grand_total)) {
                echo json_encode([
                    'success' => true, 
                    'redirect' => 'verify-order.php',
                    'message' => 'Order placed! Please verify your order with the OTP sent to your email.'
                ]);
                exit();
            } else {
                echo json_encode([
                    'success' => true, 
                    'redirect' => 'verify-order.php',
                    'message' => 'Order placed! Proceed to verification page.'
                ]);
                exit();
            }
        } else {
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .otp-info i { font-size: 18px; color: #e67e22; }
...
<div class="customer-content">
    <div class="page-header">
        ...
        <p><i class="fas fa-lock"></i> Secure checkout · You will verify your order with OTP</p>
    </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="otp-info">
                            ...
                            <div>
                                <strong>Order Verification Required</strong><br>
                                <small>After placing order, we'll send a 6-digit OTP to your email to verify your order.</small>
                            </div>
                        </div>
