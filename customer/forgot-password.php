...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['send_otp'])) {
    ...
    if (empty($email)) {
        ...
        $sql = "SELECT user_id, full_name FROM users WHERE email = '$email_escaped' AND role = 'customer' AND status = 'active'";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            mysqli_query($conn, "DELETE FROM otp_verifications WHERE email = '$email_escaped' AND purpose = 'customer_password_reset'");
            
            $expires_at = date('Y-m-d H:i:s', strtotime('+10 minutes'));
            $insert_sql = "INSERT INTO otp_verifications (email, otp_code, purpose, expires_at, created_at) 
                           VALUES ('$email_escaped', '$otp', 'customer_password_reset', '$expires_at', NOW())";
            ...
            if (sendOTPEmail($email, $otp, $user_name)) {
                ...
                $_SESSION['reset_full_name'] = $user_name;
