<?php
// admin/reports/orders.php - Orders Report
...
if (!empty($search)) {
    ...
    $where .= " AND (o.order_id LIKE '%$search_esc%' 
...
foreach ($orders as $o) {
    ...
    if ($o['status'] === 'delivered') {
        $delivered_sales += $o['grand_total'];
    } elseif ($o['status'] === 'pending') {
        $pending_sales += $o['grand_total'];
    } elseif ($o['status'] === 'cancelled') {
        $cancelled_sales += $o['grand_total'];
    }
...
// ============================================================
$top_customers_result = mysqli_query($conn, "SELECT 
                                                    CONCAT(c.first_name, ' ', c.last_name) as customer_name,
                                                    COUNT(o.order_id) as order_count,
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media (max-width: 768px) {
            ...
            .payment-chart-container { height: 150px; max-width: 200px; }
        }
...
<div class="report-content">
    ...
    <div class="page-header">
        <div>
            <h1><i class="fas fa-shopping-bag"></i> Orders Report</h1>
            <p><?php echo date('M d, Y', strtotime($date_from)); ?> - <?php echo date('M d, Y', strtotime($date_to)); ?></p>
        </div>
        <div class="header-actions">
            <a href="export.php?type=orders&from=<?php echo $date_from; ?>&to=<?php echo $date_to; ?>" class="btn-export">
    ...
    <div class="summary-grid">
        <div class="summary-card">
            ...
            <div class="sub-text">All orders in period</div>
        </div>
        <div class="summary-card">
            ...
            <div class="sub-text">Overall revenue</div>
        </div>
        <div class="summary-card">
            <div class="label"><i class="fas fa-check-circle"></i> Delivered</div>
            <div class="value green"><?php echo number_format($delivered_count); ?></div>
        ...
        <div class="summary-card">
            ...
            <div class="value red"><?php echo number_format($cancelled_count); ?></div>
    ...
    <div class="chart-section">
        <div class="chart-header">
            <h3><i class="fas fa-chart-bar"></i> Order Trends</h3>
            <div class="period-btns">
                <a href="?period=daily&from=<?php echo $date_from; ?>&to=<?php echo $date_to; ?>&status=<?php echo $status_filter; ?>&payment=<?php echo $payment_filter; ?>&search=<?php echo urlencode($search); ?>" 
                   class="period-btn <?php echo $period === 'daily' ? 'active' : ''; ?>">Daily</a>
                <a href="?period=weekly&from=<?php echo $date_from; ?>&to=<?php echo $date_to; ?>&status=<?php echo $status_filter; ?>&payment=<?php echo $payment_filter; ?>&search=<?php echo urlencode($search); ?>" 
                   class="period-btn <?php echo $period === 'weekly' ? 'active' : ''; ?>">Weekly</a>
                <a href="?period=monthly&from=<?php echo $date_from; ?>&to=<?php echo $date_to; ?>&status=<?php echo $status_filter; ?>&payment=<?php echo $payment_filter; ?>&search=<?php echo urlencode($search); ?>" 
        ...
        <div class="chart-section" style="margin-bottom:0;">
            ...
            <div style="display:flex; flex-wrap:wrap; gap:10px; justify-content:center; padding:10px 0;">
                <div style="text-align:center; flex:1; min-width:80px;">
                    <div style="font-size:24px; font-weight:800; color:#10b981;"><?php echo $delivered_count; ?></div>
                    <div style="font-size:11px; color:#64748b;">Delivered</div>
                </div>
                <div style="text-align:center; flex:1; min-width:80px;">
                    <div style="font-size:24px; font-weight:800; color:#3b82f6;"><?php echo $pending_count; ?></div>
                    <div style="font-size:11px; color:#64748b;">Pending</div>
                </div>
                <div style="text-align:center; flex:1; min-width:80px;">
                    <div style="font-size:24px; font-weight:800; color:#ef4444;"><?php echo $cancelled_count; ?></div>
                    <div style="font-size:11px; color:#64748b;">Cancelled</div>
                </div>
                <div style="text-align:center; flex:1; min-width:80px;">
                    <div style="font-size:24px; font-weight:800; color:#8b5cf6;"><?php echo $total_orders; ?></div>
    ...
    <!-- Top Businesses & Top Customers -->
    <div class="top-grid">
        <div class="top-card">
            <div class="top-card-header"><i class="fas fa-trophy"></i> Top Businesses by Orders</div>
        ...
        <div class="top-card">
            <div class="top-card-header"><i class="fas fa-users"></i> Top Customers by Orders</div>
            ...
            <?php else: ?>
                ...
                <div class="top-item">
                    <div class="top-rank rank-<?php echo $index + 1; ?>"><?php echo $index + 1; ?></div>
                    <div class="top-info">
                        <div class="name"><?php echo htmlspecialchars($c['customer_name']); ?></div>
                        <div class="detail"><?php echo $c['order_count']; ?> orders</div>
                    </div>
                    <div class="top-amount">TSh <?php echo number_format($c['total_spent']); ?></div>
                </div>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
        ...
        <form method="GET" style="display:flex; gap:15px; flex-wrap:wrap; width:100%; align-items:flex-end;">
            ...
            <div class="filter-group">
                ...
                <select name="payment" class="filter-input">
                    ...
                    <option value="tigo pesa" <?php echo $payment_filter === 'tigo pesa' ? 'selected' : ''; ?>>Tigo Pesa</option>
                    <option value="airtel money" <?php echo $payment_filter === 'airtel money' ? 'selected' : ''; ?>>Airtel Money</option>
            ...
            <div class="filter-buttons" style="display:flex; gap:10px;">
                ...
                <a href="orders.php?period=<?php echo $period; ?>" class="btn-reset"><i class="fas fa-undo-alt"></i> Reset</a>
            </div>
        </form>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var paymentData = <?php echo json_encode($payment_methods); ?>;
    var paymentLabels = Object.keys(paymentData);
    var paymentValues = Object.values(paymentData);
    
    var colors = ['#e67e22', '#3b82f6', '#10b981', '#8b5cf6', '#ef4444', '#f59e0b'];
