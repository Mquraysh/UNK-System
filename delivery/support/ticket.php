...
// AUTHENTICATION - Check if user is logged in as delivery agent
...
$source = isset($_GET['source']) ? $_GET['source'] : 'delivery';
...
$ticket_sql = "SELECT t.*, 
               ...
               CASE 
                   ...
                   WHEN t.assigned_to_type = 'delivery' AND t.assigned_to_id = $agent_id THEN 'received'
                   ELSE 'unknown'
               END as direction
               FROM support_tickets t
               WHERE t.id = $ticket_id 
               AND ((t.created_by_type = 'delivery' AND t.created_by_id = $agent_id) 
...
if (mysqli_num_rows($ticket_res) == 0) {
    $_SESSION['flash_message'] = "Ticket not found or not assigned to you.";
...
// HANDLE TICKET DELETE (Only if open/in_progress and delivery agent created it)
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['send_reply'])) {
    ...
    if (empty($reply_msg)) {
        ...
        if (empty($reply_error)) {
            ...
            $sender_name = mysqli_real_escape_string($conn, $agent_name);
            ...
            $insert = "INSERT INTO support_replies (ticket_id, reply_by_type, reply_by_id, message, attachment)
                       VALUES ($ticket_id, 'delivery', $agent_id, '$esc_msg', '$esc_att')";
...
$replies[] = [
    'reply_by_type' => $ticket['direction'] == 'sent' ? 'delivery' : 'customer',
    'display_name' => $ticket['direction'] == 'sent' ? $agent_name : 'Customer',
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .reply-avatar.delivery { background: linear-gradient(135deg, #3498db, #2980b9); }
            ...
            <div class="replies-list">
                ...
                <?php else: ?>
                    <?php foreach ($replies as $idx => $rep): 
                        ...
                        // Determine if reply is from delivery or support
                        $isDelivery = ($rep['reply_by_type'] == 'delivery');
                        $author = $isDelivery ? $agent_name : ($rep['display_name'] ?? 'Support');
                    ...
                    <div class="reply-item">
                        <div class="reply-avatar <?php echo $isDelivery ? 'delivery' : 'support'; ?>">
                            <i class="fas <?php echo $isDelivery ? 'fa-truck' : 'fa-headset'; ?>"></i>
                        </div>
                        <div class="reply-content">
                            <div class="reply-header">
                                <span class="reply-sender">
                                    <?php echo $icon; ?> <?php echo htmlspecialchars($author); ?>
                                    <span class="badge <?php echo $isDelivery ? '' : 'support'; ?>">
                                        <?php echo $isDelivery ? 'You' : 'Support Team'; ?>
                                    </span>
                                </span>
