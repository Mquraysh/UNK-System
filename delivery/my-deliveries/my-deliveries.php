<?php
// delivery/my-deliveries/my-deliveries.php - Professional Delivery Management (FIXED)
...
// ============================================
// BUILD QUERY - GET BOTH DELIVERY AND ORDER STATUS
// ============================================
$sql = "SELECT 
            d.*, 
            o.order_id, 
...
if (!empty($status_filter)) {
    // Filter by delivery status (not order status)
...
<head>
    ...
    <style>
        * {
        ...
        /* ============================================
           STATUS BADGES - BOTH DELIVERY AND ORDER
           ============================================ */
        .status-badge {
            display: inline-flex;
            align-items: center;
        ...
        .order-status-picked_up { background: #ede9fe; color: #5b21b6; }
        ...
        @media (max-width: 768px) {
            ...
            .data-table th, .data-table td { padding: 0.75rem 0.8rem; font-size: 0.7rem; }
            .btn-sm { padding: 0.3rem 0.6rem; font-size: 0.6rem; }
            
            .data-table thead { display: none; }
            .data-table tr { display: block; border-bottom: 2px solid #e2e8f0; padding: 0.75rem; margin-bottom: 0.5rem; background: white; border-radius: 0.5rem; }
            .data-table td { display: flex; justify-content: space-between; align-items: center; padding: 0.5rem 0; border-bottom: 1px solid #f1f5f9; font-size: 0.75rem; }
            .data-table td:last-child { border-bottom: none; flex-wrap: wrap; gap: 0.5rem; }
            .data-table td::before { content: attr(data-label); font-weight: 700; color: #64748b; font-size: 0.65rem; text-transform: uppercase; letter-spacing: 0.3px; }
        }
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <?php foreach($deliveries as $delivery): 
                        ...
                        // Order Status
                        $orderStatusClass = 'order-status-' . $delivery['order_status'];
                        $orderStatusIcon = '';
                        $orderStatusText = '';
                        if ($delivery['order_status'] == 'confirmed') { $orderStatusIcon = 'fa-check-circle'; $orderStatusText = 'Confirmed'; }
                        elseif ($delivery['order_status'] == 'preparing') { $orderStatusIcon = 'fa-cogs'; $orderStatusText = 'Preparing'; }
                        elseif ($delivery['order_status'] == 'ready') { $orderStatusIcon = 'fa-box-open'; $orderStatusText = 'Ready'; }
                        elseif ($delivery['order_status'] == 'picked_up') { $orderStatusIcon = 'fa-box-open'; $orderStatusText = 'Picked Up'; }
                        elseif ($delivery['order_status'] == 'in_transit') { $orderStatusIcon = 'fa-truck'; $orderStatusText = 'In Transit'; }
                        elseif ($delivery['order_status'] == 'delivered') { $orderStatusIcon = 'fa-check-circle'; $orderStatusText = 'Delivered'; }
                        elseif ($delivery['order_status'] == 'cancelled') { $orderStatusIcon = 'fa-times-circle'; $orderStatusText = 'Cancelled'; }
                        else { $orderStatusIcon = 'fa-clock'; $orderStatusText = ucfirst($delivery['order_status']); }
                    ?>
                    <tr>
                        ...
                        <td data-label="Order Status">
                            <span class="order-status-badge <?php echo $orderStatusClass; ?>">
                                <i class="fas <?php echo $orderStatusIcon; ?>"></i> <?php echo $orderStatusText; ?>
                            </span>
                        </td>
