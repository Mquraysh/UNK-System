<?php
// delivery/forgot-password.php - Delivery Agent Forgot Password (Matching Login Style)
...
function sendOTPEmail($to, $otp, $name) {
    ...
    if (file_exists($phpmailer_path)) {
        ...
        try {
            ...
            $mail->setFrom('no-reply@unksystem.com', 'UNK System Delivery');
            $mail->addAddress($to, $name);
            
            $mail->isHTML(true);
            $mail->Subject = 'Password Reset OTP - UNK System Delivery';
            $mail->Body = '
            <!DOCTYPE html>
            <html>
            <head>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['send_otp'])) {
    ...
    if (empty($email)) {
        ...
        $sql = "SELECT u.user_id, u.full_name, d.agent_id, d.first_name, d.last_name 
                FROM users u 
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            mysqli_query($conn, "DELETE FROM otp_verifications WHERE email = '$email_escaped' AND purpose = 'delivery_password_reset'");
            
            $expires_at = date('Y-m-d H:i:s', strtotime('+10 minutes'));
            $insert_sql = "INSERT INTO otp_verifications (email, otp_code, purpose, expires_at, created_at) 
                           VALUES ('$email_escaped', '$otp', 'delivery_password_reset', '$expires_at', NOW())";
            mysqli_query($conn, $insert_sql);
            
            $user_name = $user['first_name'] ? $user['first_name'] . ' ' . $user['last_name'] : $user['full_name'];
            if (sendOTPEmail($email, $otp, $user_name)) {
                $_SESSION['reset_delivery_email'] = $email;
                $_SESSION['reset_delivery_user_id'] = $user['user_id'];
                $_SESSION['reset_delivery_agent_id'] = $user['agent_id'];
                $_SESSION['reset_delivery_name'] = $user_name;
            ...
            $error = "No delivery agent account found with this email address.";
        }
    }
}
...
<head>
    ...
    <title>Forgot Password - UNK System Delivery</title>
...
<div class="login-container">
    <div class="card">
        <div class="card-header">
            <h2><i class="fas fa-truck"></i> Forgot Password</h2>
            <p>Enter your email to reset your delivery account password</p>
        </div>
        <div class="card-body">
            <?php if(!empty($error)): ?>
