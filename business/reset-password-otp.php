<?php
// business/reset-password-otp.php 
session_start();

// Redirect if no reset session data
if (!isset($_SESSION['reset_business_email']) || !isset($_SESSION['reset_business_user_id'])) {
...
$error = '';
$success = '';
$email = $_SESSION['reset_business_email'];
$full_name = $_SESSION['reset_business_name'] ?? 'Business Owner';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action === 'verify_otp') {
        ...
        if (empty($otp_code)) {
            ...
            $sql = "SELECT otp_id, expires_at, is_used FROM otp_verifications 
                    ...
                    AND purpose = 'business_password_reset' 
                    AND is_used = 0 
            ...
            if ($result && mysqli_num_rows($result) > 0) {
                ...
                if ($time_diff > 0) {
                    ...
                    $_SESSION['reset_business_verified'] = true;
    ...
    if ($action === 'resend_otp') {
        ...
        $check_sql = "SELECT otp_id, expires_at FROM otp_verifications 
                      WHERE email = '$email_escaped' AND purpose = 'business_password_reset' AND is_used = 0 
        ...
        if ($check_result && mysqli_num_rows($check_result) > 0) {
            ...
            if ($time_diff > 30) {
                ...
                mysqli_query($conn, "DELETE FROM otp_verifications WHERE email = '$email_escaped' AND purpose = 'business_password_reset' AND is_used = 0");
                
                $expires_at = date('Y-m-d H:i:s', strtotime('+10 minutes'));
                $insert_sql = "INSERT INTO otp_verifications (email, otp_code, purpose, expires_at, created_at) 
                               VALUES ('$email_escaped', '$new_otp', 'business_password_reset', '$expires_at', NOW())";
...
<div class="login-container">
    <div class="card">
        <div class="card-header">
            <h2><i class="fas fa-store"></i> Reset Password</h2>
