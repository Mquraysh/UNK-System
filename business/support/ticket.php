<?php
// ============================================================
// FILE: business/support/ticket.php
// PURPOSE: Display single ticket details, replies, and allow replies
// DEPENDENCIES: config/database.php, includes/business_sidebar.php
// ============================================================
...
// ============================================================
// AUTHENTICATION - Check if user is logged in as business
// ============================================================
...
$source = isset($_GET['source']) ? $_GET['source'] : 'business';
...
$ticket_sql = "SELECT t.*, 
               ...
               CASE 
                   WHEN t.created_by_type = 'business' AND t.created_by_id = $business_id THEN 'sent'
                   WHEN t.assigned_to_type = 'business' AND t.assigned_to_id = $business_id THEN 'received'
                   ELSE 'unknown'
               END as direction
               FROM support_tickets t
               WHERE t.id = $ticket_id 
               AND ((t.created_by_type = 'business' AND t.created_by_id = $business_id) 
...
if (mysqli_num_rows($ticket_res) == 0) {
    $_SESSION['flash_message'] = "Ticket not found or not assigned to your business.";
...
// ============================================================
if (isset($_GET['close']) && !in_array($ticket['status'], ['closed', 'resolved'])) {
    ...
    header("Location: ticket.php?id=$ticket_id&source=$source");
    exit();
}
...
// ============================================================
if (isset($_GET['reopen']) && $ticket['status'] == 'closed') {
...
// ============================================================
// HANDLE TICKET STATUS CHANGE - Resolve Ticket
// ============================================================
if (isset($_GET['resolve']) && !in_array($ticket['status'], ['resolved', 'closed'])) {
    mysqli_query($conn, "UPDATE support_tickets SET status = 'resolved', updated_at = NOW() WHERE id = $ticket_id");
    $_SESSION['flash_message'] = "Ticket #" . $ticket['ticket_no'] . " marked as resolved.";
    $_SESSION['flash_type'] = "success";
    header("Location: ticket.php?id=$ticket_id&source=$source");
    exit();
}
...
// ============================================================
// HANDLE TICKET DELETE (Only if open/in_progress and business created it)
// ============================================================
if (isset($_GET['delete']) && $_GET['delete'] == '1' && in_array($ticket['status'], ['open', 'in_progress']) && $ticket['direction'] == 'sent') {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['send_reply'])) {
    ...
    if (empty($reply_msg)) {
        ...
        if (empty($reply_error)) {
            ...
            $sender_name = mysqli_real_escape_string($conn, $business_name);
            ...
            $insert = "INSERT INTO support_replies (ticket_id, reply_by_type, reply_by_id, message, attachment)
                       VALUES ($ticket_id, 'business', $business_id, '$esc_msg', '$esc_att')";
...
$replies[] = [
    'reply_by_type' => $ticket['direction'] == 'sent' ? 'business' : 'customer',
    'display_name' => $ticket['direction'] == 'sent' ? $business_name : 'Customer',
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .reply-avatar.business { background: linear-gradient(135deg, #e67e22, #d35400); }
    ...
    <div class="dashboard-content">
        ...
        <div class="top-buttons">
            ...
            <div class="action-buttons">
                <?php if ($ticket['status'] == 'open' || $ticket['status'] == 'in_progress'): ?>
                    <a href="?id=<?php echo $ticket_id; ?>&source=<?php echo $source; ?>&resolve=1" class="btn btn-success" 
                ...
                <?php if ($ticket['status'] != 'closed' && $ticket['status'] != 'resolved'): ?>
                    <a href="?id=<?php echo $ticket_id; ?>&source=<?php echo $source; ?>&close=1" class="btn btn-danger" 
                       onclick="return confirm('Close this ticket? You can reopen it later.')">
                        <i class="fas fa-times-circle"></i> Close Ticket
                    </a>
                <?php endif; ?>
                
                <?php if ($ticket['status'] == 'closed'): ?>
                    <a href="?id=<?php echo $ticket_id; ?>&source=<?php echo $source; ?>&reopen=1" class="btn btn-primary" 
                       onclick="return confirm('Reopen this ticket?')">
                        <i class="fas fa-folder-open"></i> Reopen Ticket
                    </a>
                <?php endif; ?>
                
                <?php if (in_array($ticket['status'], ['open', 'in_progress']) && $ticket['direction'] == 'sent'): ?>
                    <a href="?id=<?php echo $ticket_id; ?>&source=<?php echo $source; ?>&delete=1" class="btn btn-secondary" 
                       style="background: #fee2e2; color: #dc2626; border-color: #fecaca;" 
                       onclick="return confirm('Delete this ticket permanently? This action cannot be undone.')">
            ...
            <div class="replies-list">
                <?php if (count($replies) == 1): ?>
                    <div class="empty-state">
                        ...
                        <p>No replies yet. The assigned party will respond shortly.</p>
                ...
                <?php else: ?>
                    <?php foreach ($replies as $idx => $rep): 
                        ...
                        // Determine if reply is from business or support
                        $isBusiness = ($rep['reply_by_type'] == 'business');
                        $author = $isBusiness ? $business_name : ($rep['display_name'] ?? 'Support');
                    ...
                    <div class="reply-item">
                        <div class="reply-avatar <?php echo $isBusiness ? 'business' : 'support'; ?>">
                            <i class="fas <?php echo $isBusiness ? 'fa-store' : 'fa-headset'; ?>"></i>
                        </div>
                        <div class="reply-content">
                            <div class="reply-header">
                                <span class="reply-sender">
                                    <?php echo $icon; ?> <?php echo htmlspecialchars($author); ?>
                                    <span class="badge <?php echo $isBusiness ? '' : 'support'; ?>">
                                        <?php echo $isBusiness ? 'You' : 'Support Team'; ?>
                                    </span>
                                </span>
        ...
        <?php elseif ($ticket['status'] == 'resolved'): ?>
            <div class="alert alert-info" style="margin-top: 0;">
                ...
                This ticket has been resolved. You can <strong>close</strong> it using the button above.
            </div>
        <?php else: ?>
            <div class="alert alert-info">
                <i class="fas fa-lock"></i> 
                This ticket is <strong>closed</strong>. You can <strong>reopen</strong> it using the button above if you need further assistance.
            </div>
        <?php endif; ?>
    </div>
</div>
